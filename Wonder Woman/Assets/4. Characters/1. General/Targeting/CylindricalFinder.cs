...
{
    [CreateAssetMenu(fileName = "Cylindrical Finder", menuName = "Targeting/Finders/Cylindrical Finder")]
    public class CylindricalFinder : TargetFinder
    {
        [SerializeField] private TargetableList _targetableList;
        [SerializeField] private float _maxRadius = 10f;
        [SerializeField] private float _maxVerticalSeparation = 10f;
        ...
        public override bool Evaluate(ISearcher hunter, Targetable targetable)
        {
            ...
            Vector3 displacement = hunter.Position - targetable.Position;

            if (Mathf.Abs(displacement.y) > _maxVerticalSeparation)
                return false;

            float horizontalSqrDistance = displacement.x*displacement.x + displacement.z*displacement.z;
            if (horizontalSqrDistance > _maxRadius)
                return false;

            if (IsViewObstructed(hunterPosition: hunter.Position, targetPosition: targetable.Position, distance: displacement.magnitude))
                return false;
